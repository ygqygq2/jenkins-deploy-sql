function Backup_Db_Surreal() {
  ...
  echo "####################### 备份[$db]开始 ##########################"
  Save_Log "备份[$db]"
  $SURREAL_PROG export --conn $SURREAL_HOST:$SURREAL_PORT --user $SURREAL_USER --pass $SURREAL_PASSWORD \
...
function Check_Db_Surreal() {
  # 暂时还没有查询方法
  local db=$1
  $SURREAL_PROG --conn $SURREAL_HOST:$SURREAL_PORT --user $SURREAL_USER --pass $SURREAL_PASSWORD \
...
function Deploy_Db_Surreal() {
  ...
  for db_dir in $(ls | sort -d); do
    ...
    for sql in $(find $db_dir -name *.surql -type f | sort -n); do
      echo "####################### 导入 [$db] < $sql ##########################"
      Save_Log "导入[$d] < $sql"
      ${SURREAL_PROG} import --conn $SURREAL_HOST:$SURREAL_PORT \
